...
import { selectMetaAdsAccountAction } from "@/server/integrations/meta/actions";

interface SelectMetaAdAccountFormProps {
  clientId: string;
  accounts: AccessibleAccount[];
}

export function SelectMetaAdAccountForm({ clientId, accounts }: SelectMetaAdAccountFormProps) {
  ...
  async function handleConfirm() {
    ...
    setSubmitting(true);
    setError(null);
    const result = await selectMetaAdsAccountAction(clientId, account);
    setSubmitting(false);
    ...
    router.push(`/clientes/${clientId}/meta-ads`);
  }
