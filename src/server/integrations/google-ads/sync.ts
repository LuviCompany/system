...
import { getGoogleAdsConnection, getValidAccessToken, markSyncing, markSyncResult } from "./connection.service";
import { GoogleAdsError, toGoogleAdsError } from "./errors";
...
function mapCampaignStatus(googleStatus: string): CampaignStatus {
  if (googleStatus === "PAUSED") return "PAUSADA";
  if (googleStatus === "REMOVED" || googleStatus === "ENDED") return "ENCERRADA";
  return "ATIVA";
}
...
async function persistCampaignMetrics(
  ...
  for (const [externalCampaignId, group] of byCampaign) {
    ...
    // Um cliente real (conectado de verdade) nunca tem métricas mock — apagar
    // e recriar a janela sincronizada evita duplicar linhas a cada clique em
    // "Sincronizar agora" (etapa 19 — preferir dados sincronizados do banco).
...
async function persistAdMetrics(
  ...
  for (const [externalAdId, group] of byAd) {
    ...
    if (!campaignId) {
      ...
      // ex: diferença de fuso na fronteira do período) — garante a campanha
      // antes de gravar o anúncio.
      const campaign = await prisma.campaign.upsert({
...
export type SyncResult = { ok: true; campaigns: number; ads: number } | { ok: false; error: string };

/** Etapa 18 — "Sincronizar agora": busca campanhas + anúncios dos últimos 30 dias e regrava no PostgreSQL. Somente leitura na API do Google. */
export async function syncGoogleAdsForClient(organizationId: string, clientId: string): Promise<SyncResult> {
  await markSyncing(organizationId, clientId);

  try {
    const connection = await getGoogleAdsConnection(organizationId, clientId);
    if (!connection?.externalAccountId) {
      throw new GoogleAdsError("UNAUTHORIZED", "Nenhuma conta Google Ads selecionada para este cliente.");
    }

    const { accessToken, loginCustomerId } = await getValidAccessToken(organizationId, clientId);
    const period = syncWindow();
    const metadata = loginCustomerId ? { loginCustomerId } : undefined;

    const [campaignRows, adRows] = await Promise.all([
      googleAdsProvider.fetchCampaignMetrics({ externalAccountId: connection.externalAccountId, accessToken, period, metadata }),
      googleAdsProvider.fetchAdMetrics({ externalAccountId: connection.externalAccountId, accessToken, period, metadata }),
    ]);
    ...
    await markSyncResult(organizationId, clientId, { ok: true });
    ...
    await markSyncResult(organizationId, clientId, { ok: false, error: mapped.friendlyMessage });
