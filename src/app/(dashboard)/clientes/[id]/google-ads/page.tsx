...
import { SyncGoogleAdsButton } from "@/components/clientes/sync-google-ads-button";
...
/** Dashboard real de Google Ads (etapa 5, seções 12-16) — só existe dado aqui depois de "Sincronizar agora". */
export default async function ClienteGoogleAdsPage({ params, searchParams }: PageProps) {
  ...
  const baseFilter = { organizationId: session.organizationId, clientId: id, period, platform: "GOOGLE_ADS" as const };
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          {connected ? (
            <SyncGoogleAdsButton clientId={id} />
          ) : (
            <Button asChild>
              <Link href={`/clientes/${id}/integracoes/google`}>Conectar Google Ads</Link>
            </Button>
          )}
        </div>
      </div>
      ...
      {!connected ? (
        <EmptyState
          ...
          description="Conecte a conta Google Ads deste cliente para ver campanhas, anúncios e métricas reais aqui."
          action={
            <Button asChild>
              <Link href={`/clientes/${id}/integracoes/google`}>Conectar Google Ads</Link>
            </Button>
        ...
        <>
          {metrics && <PlatformSummaryCard title="Resumo (Google Ads)" connected activeCampaigns={campaigns.filter((c) => c.status === "ATIVA").length} metrics={metrics} />}
